...
bool oplus_get_usb_online(void)
{
	...
	if (rc < 0) {
		chg_err("read usb usbonline_status fail, rc=%d\n", rc);
		return false;
	}
	usbonline_status = pst->prop[USB_ONLINE];
	chg_err("read usbonline_status[%d]\n", usbonline_status);
	return usbonline_status;
}
